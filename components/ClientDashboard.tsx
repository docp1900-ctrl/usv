        ...
        <li key={transfer.id} className={`p-4 rounded-lg ${isBlocked ? 'bg-error/10' : 'bg-base-200'}`}>
            <div className="flex justify-between items-start">
                ...
                <div className="text-right">
                    <p className={`font-bold text-lg ${isBlocked ? 'text-error' : 'text-neutral'}`}>{formatCurrency(transfer.amount)}</p>
                    <p className={`text-sm font-semibold capitalize badge ${isBlocked ? 'badge-error' : transfer.status === 'completed' ? 'badge-success' : 'badge-ghost'}`}>{t(transfer.status)}</p>
                </div>
            </div>
            {isBlocked && (
                <div className="mt-4">
  ...
  const renderCreditRequestItem = (request: CreditRequest) => (
      <li key={request.id} className="flex justify-between items-center p-4 bg-base-200 rounded-lg">
        ...
        <div className="text-right">
            ...
            <p className={`text-sm font-semibold capitalize badge ${request.status === 'approved' ? 'badge-success' : request.status === 'rejected' ? 'badge-error' : 'badge-ghost'}`}>{t(request.status)}</p>
        </div>
      </li>
  );
    ...
    <div className="space-y-8">
      <Card>
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-neutral">{t('welcome_user', { name: user.name })}</h1>
      ...
      <div className="flex gap-4">
        <Button onClick={() => setTransferModalOpen(true)} className="flex-1">{t('new_transfer')}</Button>
        <Button onClick={() => setCreditModalOpen(true)} variant="secondary" className="flex-1">{t('request_credit')}</Button>
      </div>

      <Card title={t('recent_activity')}>
         <div role="tablist" className="tabs tabs-bordered mb-4">
            <a role="tab" className={`tab ${activeTab === 'transfers' ? 'tab-active' : ''}`} onClick={() => setActiveTab('transfers')}>{t('transfers')}</a>
            <a role="tab" className={`tab ${activeTab === 'credits' ? 'tab-active' : ''}`} onClick={() => setActiveTab('credits')}>{t('credit_requests')}</a>
        </div>
        
        {activeTab === 'transfers' && (
            <ul className="space-y-4">
                {transfers.length > 0 ? transfers.map(renderTransferItem) : <p className="text-center p-4">{t('no_transfers_found')}</p>}
            </ul>
        )}
        
        {activeTab === 'credits' && (
            <ul className="space-y-4">
                {creditRequests.length > 0 ? creditRequests.map(renderCreditRequestItem) : <p className="text-center p-4">{t('no_credit_requests_found')}</p>}
            </ul>
        )}
      </Card>

      <Modal isOpen={isTransferModalOpen} onClose={() => setTransferModalOpen(false)} title={t('new_transfer')}>
        <TransferForm userId={user.id} onNewTransfer={onNewTransfer} closeModal={() => setTransferModalOpen(false)} />
      </Modal>

      <Modal isOpen={isCreditModalOpen} onClose={() => setCreditModalOpen(false)} title={t('request_credit')}>
        <CreditForm userId={user.id} onNewCreditRequest={onNewCreditRequest} closeModal={() => setCreditModalOpen(false)} />
      </Modal>
      
      {/* Chat Window */}
      {!isChatOpen && (
          <button onClick={() => setChatOpen(true)} className="fixed bottom-6 right-6 btn btn-primary btn-circle btn-lg shadow-lg">
        ...
        <div className="fixed bottom-4 right-4 z-50 w-full max-w-sm">
           <Card className="!p-0">
             <div className="flex justify-between items-center p-4 border-b">
                ...
                <Button size="sm" variant="ghost" className="!p-1" onClick={() => setChatOpen(false)}>✕</Button>
             </div>
